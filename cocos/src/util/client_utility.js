...
//————————————————————东阳四副牌————————————
//获取同样牌的张数 dict
// cutil.getTileNumDict
cutil.rightShiftCards = function(cards) {
    //把牌做个右移三位，比如[25,26,27,28]——>[3,3,3,3]
    var result = [];
...
// 0无牌 1非可出牌型 2单张 3对子 4三张 5炸弹
cutil.getNormalCardsType = function(cards) {
    // cc.log("getNormalCardsType 获得牌的类型");
    if (cards.length <= 0) {
...
cutil.checkIsSingle = function(cards) {
    // cc.log("checkIsSingle 检测是否是单张:", cards);
    if (cards.length == 1) {
        return true;
    }
    return false;
...
cutil.checkIsPair = function(cards) {
    // cc.log("checkIsPair 检测是否是对子:", cards);
...
cutil.checkIsTriple = function(cards) {
    // cc.log("checkIsTriple 检测是否是三张:", cards);
...
cutil.checkIsNormalBomb = function(cards) {
    // cc.log("checkIsNormalBomb 检测是否是炸弹:", cards);
    if (cards && cards[0] > const_val.FANG[0]) {
        cards = cutil.rightShiftCards(cards);
	}
    var card2NumDict = cutil.getTileNumDict(cards);
    if (cards.length >= 4 && (Object.keys(card2NumDict).length == 1 || (card2NumDict[18] && card2NumDict[19] && Object.keys(card2NumDict).length == 2))) {
        return true;
    }
    return false;
};
...
cutil.cmpSameTypeCards = function(baseCards, selCards, cardsType) {
    // cc.log("牌进行比较 cmpSameTypeCards",baseCards, selCards, cardsType);
    if (cardsType == 0 || cardsType == 1) {
        return false;
    } else if (cardsType == 2 || cardsType == 3 || cardsType == 4) {
        if (baseCards.length == selCards.length && selCards[0] > baseCards[0]) {
            return true
        }
    } else if (cardsType == 5) {
    	var selCardsSize = selCards[0];			//出牌的炸弹的大小
    	var selCardsLen = selCards.length;		//出牌的炸弹的长度
    	var baseCardsSize = baseCards[0];		//上一次出牌的炸弹的大小
    	var baseCardsLen = baseCards.length;	//上一次出牌的炸弹的长度
        if (selCards[0] >= 18) {
        	//如果出牌是王炸
        	selCardsSize = cutil.getKingSizeLen(selCards)[0];
            selCardsLen = cutil.getKingSizeLen(selCards)[1];
        }
        if (baseCards[0] >= 18) {
            //如果上一次出牌是王炸
            baseCardsSize = cutil.getKingSizeLen(baseCards)[0];
            baseCardsLen = cutil.getKingSizeLen(baseCards)[1];
        }
        if (selCardsLen == baseCardsLen) {
            if (selCardsSize > baseCardsSize) {
                return true
            }
        } else if (selCardsLen > baseCardsLen) {
            return true
        }
    }
    return false
...
cutil.getKingSizeLen = function(tile){
	//这里是出大小王炸时的处理，Size代表大小，Len代表长度，返回值为（大小，长度）
	if (tile[0] < 17) {
        return [tile[0], tile.length];
    }
    if (tile && tile[0] > const_val.FANG[0]) {
		tile = cutil.rightShiftCards(tile);
	}
    var kingDict = cutil.getTileNumDict(tile);
	cc.log("kingDict:",kingDict);
    var kingSmallNum = kingDict[const_val.JOKER[0] >> 3] || 0;
    var kingBigNum = kingDict[const_val.JOKER[1] >> 3] || 0;
    if (kingBigNum == 4) {
        if (kingSmallNum == 4) {
            return [18,16]
        } else if (kingSmallNum == 3) {
            return [19,14]
        } else if (kingSmallNum == 2) {
            return [19,12]
        } else if (kingSmallNum == 1) {
            return [19,10]
        } else if (kingSmallNum == 0) {
            return [19,8]
        }
	} else if (kingBigNum == 3) {
        if (kingSmallNum == 4) {
            return [18,14]
        } else if (kingSmallNum == 3) {
            return [18,9]
    ...
    } else if (kingBigNum == 1) {
        if (kingSmallNum == 4) {
            return [18,10]
        } else if (kingSmallNum == 3) {
            return [19,7]
        }
    } else if (kingBigNum == 0) {
        if (kingSmallNum == 4) {
            return [18,8]
        }
    }
...
cutil.subsectionCards = function (cards) {
	//给牌分段，如[25,26,33,34]-->[[25,26],[33,34]]
    var cardsDict = cutil.getTileNumDict(cutil.rightShiftCards(cards));
    var subList = [];
	for (var i in cardsDict) {
        if (cardsDict[18] + cardsDict[19] >= 4 && (i == 18 || i == 19)) {
            subList = subList.concat([cards.splice(0, cardsDict[18] + cardsDict[19]),]);
		} else {
            subList = subList.concat([cards.splice(0, cardsDict[i]),]);
        }
...
cutil.cardSubSort = function (cards) {
	//分段排序，如[[25,26,26],[34,34],[41,41]]-->[[25,26,26],[41,41],[34,34]]
	cc.log("———————————————————cutil.cardSort——————start");
	var cardsCopy = cutil.deepCopy(cards);
	var subList = cutil.subsectionCards(cardsCopy);	//牌分段
    var kingList = [];
    cc.log("subList:",subList);
    if (subList.length !== 0) {
        if (subList && subList[subList.length - 1].length >= 4 && subList[subList.length - 1][0] >= const_val.JOKER[0]) {
            kingList = subList.pop();		//如果有王炸，就分离出来
            kingList = cutil.rightShiftCards(kingList);
            // cc.log("kingList:",kingList);
        }
    }

    var subListRightShift = [];			//分段的牌右移
    for (var i = 0 ; i < subList.length ; i++) {
        subListRightShift = subListRightShift.concat([cutil.rightShiftCards(subList[i]),]);
    }
    // cc.log("subListRightShift:",subListRightShift);


    var notBombList = [];				//非炸弹list
    var bombList = [];					//炸弹list
    for (var i = 0 ; i < subListRightShift.length ; i++) {
    	//把炸弹和非炸弹分开
    	var cardType = cutil.getNormalCardsType(subListRightShift[i]);
    	if (cardType < 5) {
            notBombList = notBombList.concat([subListRightShift[i],]);
		} else {
            bombList = bombList.concat([subListRightShift[i],]);
		}
    }
    for (var j = 0 ; j < notBombList.length - 1 ; j++) {
        for (var i = 0; i < notBombList.length - 1 ; i++) {
            //非炸弹list排序
            var card1Type = cutil.getNormalCardsType(notBombList[i]);
            var card2Type = cutil.getNormalCardsType(notBombList[i + 1]);
            if (card1Type == card2Type) {
                if (cutil.cmpSameTypeCards(notBombList[i], notBombList[i + 1], card1Type)) {
                    var temp_list = notBombList[i];
                    notBombList[i] = notBombList[i + 1];
                    notBombList[i + 1] = temp_list;
                }
    ...
    // cc.log("notBombList:",notBombList);
    for (var j = 0 ; j < bombList.length - 1 ; j++) {
        for (var i = 0; i < bombList.length - 1; i++) {
            //炸弹list排序
            var card1Type = cutil.getNormalCardsType(bombList[i]);
            var card2Type = cutil.getNormalCardsType(bombList[i + 1]);
            if (card1Type == card2Type) {
                if (cutil.cmpSameTypeCards(bombList[i], bombList[i + 1], card1Type)) {
                    var temp_list = bombList[i];
                    bombList[i] = bombList[i + 1];
                    bombList[i + 1] = temp_list;
                }
            } else if (bombList[i].length < bombList[i + 1].length) {
                var temp_list = bombList[i];
                bombList[i] = bombList[i + 1];
                bombList[i + 1] = temp_list;
            }
    ...
    // cc.log("bombList:",bombList);

    var mergeList = bombList.concat(notBombList);
    if (kingList.length > 0) {
        for (var z = 0 ; z < mergeList.length ; z ++) {
            if (cutil.cmpSameTypeCards(mergeList[z], kingList, const_val.TYPE_BOMB)) {
            	mergeList.splice(z, 0, kingList);
            	break;
			}
		}
	}
    cc.log("mergeList:", mergeList);
    cc.log("———————————————————cutil.cardSort——————end");
    return mergeList;
};

cutil.cardSort = function (cards) {
	//牌按照从大到小排序
	if (cards && cards.length == 0) {
		return []
	}
	var mergeList = cutil.cardSubSort(cards);
    var cardsList = [];
    for (var n = 0 ; n < mergeList.length ; n++) {
        cardsList = cardsList.concat(mergeList[n]);
    }
    cc.log("cardsList:",cardsList);

    var realCards = [];
    if (cardsList.length == cards.length) {
        for (var l = 0; l < cardsList.length;) {
            for (var t = 0; t < cards.length; t++) {
                if ((cards[t] >> 3) == cardsList[l]) {
                    realCards.push(cards[t]);
                    l++;
                }
            }
        }
    }
    cc.log("realCards:",realCards);
    return realCards;
};

cutil.getDiscardScore = function (cards) {
    if (cards && cards.length <= 0) {
        return 0;
    }
    if (cards && cards[0] > const_val.FANG[0]) {
    ...
    var score = 0;
    if (cards[0] == 5 || cards[0] == 10 || cards[0] == 13) {
        score = cards[0] >= 10 ? 10 : 5;
    }
    return score * cards.length;
};

cutil.getBombScore = function (cards) {
    if (cards && cards.length < 4) {
        return 0;
    }
    if (cards[0] > const_val.FANG[0]) {
        cards = cutil.rightShiftCards(cards);
    }
    var bLen = cards.length;
    if (cards[0] >= (const_val.JOKER[0] >> 3)) {
        bLen = cutil.getKingSizeLen(cards)[1];
    }
    if (bLen < 7 && bLen > 16) {
        return 0;
    }
...
cutil.get_club_share_desc = function (room_info) {
    var str_list = [];
    str_list.push((room_info["maxRound"] || room_info["game_round"]) + "局");
