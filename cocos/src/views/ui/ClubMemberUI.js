...
var ClubMemberUI = UIBase.extend({
    ...
    show_by_info:function (club_id) {
        if(!h1global.entityManager.player().club_entity_dict[club_id]){return}
        this.club = h1global.entityManager.player().club_entity_dict[club_id];
        this.show();
    },
    ...
    initUI:function () {
        ...
        h1global.entityManager.player().clubOperation(const_val.CLUB_OP_GET_MEMBERS, this.club.club_id);
    ...
    update_club_member:function (member_list) {
        ...
        function update_item_func(itemPanel, itemData, index) {
                ...
                if(eventType === ccui.Widget.TOUCH_ENDED){
                    if(h1global.curUIMgr.editor_ui && !h1global.curUIMgr.editor_ui.is_show){
                        h1global.curUIMgr.editor_ui.show_by_info(function (editor_string) {
                            h1global.entityManager.player().clubOperation(const_val.CLUB_OP_SET_MEMBER_NOTES, self.club.club_id, [itemData["userId"], editor_string]);
            ...
            itemPanel.getChildByName("delete_btn").addTouchEventListener(function (sender, eventType) {
                if(eventType === ccui.Widget.TOUCH_ENDED){
                    h1global.entityManager.player().clubOperation(const_val.CLUB_OP_KICK_OUT, self.club.club_id, [itemData["userId"]]);
                }
            });
            if(h1global.entityManager.player().userId === self.club.owner.userId){
    ...
    update_club_apply:function (apply_list) {
        ...
        function update_item_func(itemPanel, itemData, index) {
            ...
            itemPanel.getChildByName("agree_btn").addTouchEventListener(function(sender, eventType){
                if(eventType === ccui.Widget.TOUCH_ENDED){
                    if(h1global.curUIMgr.editor_ui && !h1global.curUIMgr.editor_ui.is_show){
                        h1global.entityManager.player().clubOperation(const_val.CLUB_OP_AGREE_IN, self.club.club_id, [itemData["userId"]]);
                    }
                }
            });
            ...
            itemPanel.getChildByName("cancel_btn").addTouchEventListener(function (sender, eventType) {
                if(eventType === ccui.Widget.TOUCH_ENDED){
                    h1global.entityManager.player().clubOperation(const_val.CLUB_OP_REFUSE_IN, self.club.club_id, [itemData["userId"]]);
                }
            });
        }
