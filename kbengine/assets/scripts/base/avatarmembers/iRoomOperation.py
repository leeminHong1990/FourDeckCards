...
class iRoomOperation(object):
			...
			if room.room_type == const.AGENT_ROOM:
				if room.pay_mode == const.AGENT_PAY_MODE:
					...
					if switch.DEBUG_BASE:
						...
						utility.update_card_diamond(self.accountName, -card_cost, -diamond_cost, callback, "FourDeckCards RoomID:{}".format(room.roomID))  # reason 必须为英文
	...
	def postOperation(self, idx, aid, tile_list, curround_score = 0, curScoreList = []):
		if getattr(self, 'client', None):
			self.client.postOperation(idx, aid, tile_list, curround_score, curScoreList)
	...
	def waitForOperation(self, idx, aid, round_start):
		if getattr(self, 'client', None):
			self.client.waitForOperation(idx, aid, round_start)
