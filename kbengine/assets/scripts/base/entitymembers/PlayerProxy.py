...
class PlayerProxy(object):
	...
	def discardTile(self, tile = None):
		...
		self.tiles = [x for x in self.tiles if x not in tile]	#将打出的牌从数组中删除
		# self.tiles.remove(tile)
		self.owner.last_player_idx = self.idx
		self.owner.op_pass_flag = 0
		self.owner.all_discard_tiles.append(tile)
		self.owner.round_start = False
		self.owner.controller_discard_list = tile
		...
		self.owner.broadcastOperation(self.idx, const.OP_DISCARD, tile, self.owner.curround_score, self.owner.curScoreList) #没倒计时模式广播给所有人
		# self.mb.postOperation(self.idx, const.OP_DISCARD, tile)
		if len(self.tiles) == 0:
			if len(self.owner.win_list) == 2 and self.owner.curround_score > 0:
				DEBUG_MSG("room:{},curround:{} Player:{} discardTile:{} curround_score:{}".format(self.owner.roomID, self.owner.current_round, self.idx, tile, self.owner.curround_score))
				self.add_score(self.owner.curround_score)
				self.owner.curScoreList[self.idx] += self.owner.curround_score
				self.owner.curround_score = 0
			DEBUG_MSG("room:{},curround:{} Player:{} discardTile:{} OP_WIN_LIST:{} win_list:{}".format(self.owner.roomID, self.owner.current_round, self.idx, tile, const.OP_WIN_LIST[len(self.owner.win_list)], self.owner.win_list))
			self.owner.winGame(self.idx, const.OP_WIN_LIST[len(self.owner.win_list)], tile, 0, 0, [])
		self.mb.notifyChangeController((self.idx + 1) % 4)
